...
def getSeq_P(especie, acc_id):
    rec = SeqIO.parse("./static/docs/Proteinas/"+especie+".fasta", "fasta")
    for seq in rec:
        if seq.id == acc_id:
...
def getSeq_G(especie, acc_id):
    rec = SeqIO.parse("./static/docs/Genes/"+especie+".fasta", "fasta")
    for seq in rec:
        if seq.id == acc_id:
            seqID = '>' + str(seq.description)
            seqSeq = str(seq.seq)
...
def importar_homologos_G(file):
    file = "./static/docs/Homologos/Nucleotidos" + file
...
def importar_homologos_P(file):
    file = "./static/docs/Homologos/Proteinas" + file
    rec = SeqIO.parse(file, "fasta")
    out_accession = []
    out_description = []
    for r in rec:
